...
private:
	...
	void			searchLeaf(elementd*, list**, int size);							// insert node i according to binary search property
	list*		searchLeft(elementd*, list**, int size);									// return leftsubtree
	list*		searchRight(elementd*, list**, int size);									// return rightsubtree
	...
	double			computeKL(const int, const short int, const int, const short int);  // compute number of edges between two internal subtrees
	list*			getRight(const int, const short int);  // compute number of edges between two internal subtrees
	elementd*		findCommonAncestor(list**, const int, const int);						// find internal node of D that is common ancestor of i,j
	elementd* 	getRandomInternalNode();
...
// ********************************************************************************************************
elementd* dendro::getRandomInternalNode(){return &internal[(int)(floor((double)(n-1)*mtr.randExc()))]; }
...
void dendro::searchLeaf(elementd* x, list** paths, int size) {
	cout << "INTERNAL: " << x->index << endl;
	elementd* leftcurrent = x-> L;
	elementd* rightcurrent = x -> R;
	if (x->L->type == GRAPH){
		cout << "LEFT: " << x->L->index << endl;
	}

	if (x->R->type == GRAPH){
		cout << "RIGHT: " << x->R->index << endl;
	}

	// cout << "LEFT INDEX: " << leftcurrent->index << " type: " << leftcurrent->type << ", RIGHT INDEX: " << rightcurrent->index << " type: " << leftcurrent->type << endl;
	...
	for(int i =0; i < size; i++){
		...
		while (!stop){
			...
			if (head -> x == leftcurrent->index && leftcurrent->type == 0){
				cout << "LEFT: " << i << endl;
				stop = true;
			}else if (head -> x == rightcurrent -> index && rightcurrent->type == 0)
			{	
				cout << "RIGHT: " << i << endl;
				stop = true;
			}else{
...
list* dendro::searchLeft(elementd* x, list** paths, int size) {
	list *res, *tail, *newlist;
	elementd* leftcurrent = x-> L;
	res = tail;
	if (x->L->type == GRAPH){
		// cout << "LEFT: " << x->L->index << endl;
...
list* dendro::searchRight(elementd* x, list** paths, int size) {
	list *res, *tail, *newlist;
	elementd* rightcurrent = x-> R;
	res = tail;
	if (x->R->type == GRAPH){
	...
	for(int i =0; i < size; i++){
		...
		while (!stop){
			// cout << "HEAD: " << head -> x << endl;
			if (head -> x == rightcurrent->index && rightcurrent->type == 0){
				// cout << "LEFT: " << i << endl;
				newlist    = new list;				// add this node to the path
				newlist->x = i;
				tail->next = newlist;
 ...
 */
	...
	// we assign each of them the p va  ir parent, perturbed slightly so as to preserve
	...
	for (int i=0; i<n; i++) {
		...
		while (curr != NULL) {
			...
			//cout << i << " + " << curr -> x << ": " << ancestor->index << endl;
			ancestor->e += 1;
			curr		   = curr->next;
		}
	}
	...
	// for (int i = 0; i < n-1; i++){
	// 	searchLeaf(&internal[i], paths, n);
	// }
	
	// list* check;
	// check = getLeft(1, internal[1].type);
	// while (check->next){
	// 	cout << "CHECK: " << check->next->x << endl;
	// 	check = check->next;
	// }
	// for (int i = 0; i < n-1; i++){
	// 	updateInternal(i, internal[i].type);
	// }

	// while(internal[1].left->next){
	// 	cout << "LEFT: " << internal[1].left->x << endl;
	// }
	// while(internal[1].right->next){
	// 	cout << "RIGHT: " << internal[1].left->x << endl;
	// }
	// cout << endl;

	// for (int i = 0; i < n-1; i++){
	// 	cout << "Internal: " << i << endl;
	// 	while(internal[i].left->next){
	// 		cout << "LEFT: " << internal[i].left->x << endl;
	// 	}
	// 	while(internal[i].right->next){
	// 		cout << "RIGHT: " << internal[i].right->x << endl;
	// 	}
	// 	cout << endl;
	// }
	...
	for (int i = 0; i < n-1; i++){
		// sum = 0;
		nM = internal[i].L->n*internal[i].R->n;
		// cout << "nM: " << nM << " and Internal: " << i << endl;
		// left = searchLeft(&internal[i], paths, n);
		// right = searchRight(&internal[i], paths, n);
		// while (left->next){
		// 	dummy = right;

		// 	while (dummy->next){
		// 		sum += g->getWeight(left->next->x, dummy->next->x);
		// 		dummy = dummy->next;
		// 	}
		// 	left = left->next;
		// }
		sum = computeKL(internal[i].L->index, internal[i].L->type, internal[i].R->index, internal[i].R->type);
		dL = sum / nM;
		internal[i].logL = dL;
		L += dL;
		// cout << endl;
...
//		if (n<100) { printDendrogram(); }
	...
	char pauseme;
	// for (int i = 0; i <(n-1); i++){
	// 	cout << "INTERNAL " << i << ": " << internal[i].logL << endl;
	// }
	for (int i=0; i<(n-1); i++) {
...
// ********************************************************************************************************
double dendro::computeKL(const int a, const short int atype, const int b, const short int btype) {
...
//						cout << "A exit: reached null parent" << endl;
	...
	edge* current;
	int*  treeListL, *treeListR;

	treeListL = subtreeL.returnArrayOfKeys();	// subtreeL is smaller
	treeListR = subtreeR.returnArrayOfKeys();
	for (int i=0; i<nA; i++) {
		for ( int j = 0; j < nB; j++){
			result += g->getWeight(treeListL[i], treeListR[j]);
		}
	...
	for (int i=0; i<nB; i++) { subtreeR.deleteItem(treeListR[i]); }
	delete [] treeListR;
	return result;
}
// ********************************************************************************************************
	...
	for (int i=0; i<n; i++) {			// O((h+d)*n^2) - h: height of D; d: average degree in G
		for (int j=(i+1); j<n; j++) {		// decide neighbors of v_i
			...
			if (mtr.randExc() < commonAncestor->p) { 
				if (!(g->doesLinkExist(i,j))) { if (!(g->addLink(i,j, 1))) { cout << "Error: (" << j << " " << i << ")" << endl; } }
				if (!(g->doesLinkExist(j,i))) { if (!(g->addLink(j,i, 1))) { cout << "Error: (" << j << " " << i << ")" << endl; } }
			}
		}
	}
	g->printPairs();
...
bool dendro::monteCarloMove(double& delta, bool& ftaken) {
	...
	elementd	*temp, *tempe, *dummy;
	ipair	*tempPair;
	int		x, y, token, e_x, e_y, n_i, n_j, n_k, n_x, n_y, tempi;
	double kl, kl_y, kl_x;
	short int t;
	double    p, p_x, p_y, L_x, L_y, L_new, dLogL;
	char		pauseme;
	
	delta    = 0.0;
	ftaken   = false;
	// dummy = getRandomInternalNode();
	...
	if (t == LEFT) {								// 
		if (mtr.randExc() < 0.5) {					// ## LEFT ALPHA move: ((i,j),k) -> ((i,k),j)
			...
			n_y  = n_i*n_k;
			kl_y = computeKL(internal[y].L->index, internal[y].L->type, internal[x].R->index, internal[x].R->type);
			...
			// else						{ L_y = kl_y/(double) n_y; }
			L_y = kl_y/(double) n_y;

			n_x  = (n_i+n_k)*n_j;
			kl_x = computeKL(internal[y].L->index, internal[y].L->type, internal[y].R->index, internal[y].R->type) + computeKL(internal[x].R->index, internal[x].R->type, internal[y].R->index, internal[y].R->type);
			e_x  = internal[x].e + internal[y].e - e_y;						// e_yj
			p_x  = (double) e_x / (double) n_x;
			// if (e_x == 0 || e_x == n_x)	{ L_x = 0.0; }
			// else						{ L_x = kl_x / (double) n_x; }
			L_x = kl_x / (double) n_x;
			// cout << "1. " << L_x << " " << L_y << endl;
						
			dLogL = (L_x - internal[x].logL) + (L_y - internal[y].logL);
			p = min(1.00, exp(L)/exp(L+dLogL));
			// if ((dLogL < 0.0) || (mtr.randExc() < exp(dLogL))) {  // make LEFT ALPHA move
			if ((dLogL < 0.0) || (mtr.randExc() < p)) {  // make RIGHT BETA move
				ftaken = true;
				...
				if (internal[x].label > internal[x].L->label || internal[y].label > internal[y].L->label) {
					printDendrogram();
			...
			kl_y = computeKL(internal[y].R->index, internal[y].R->type, internal[x].R->index, internal[x].R->type);
			p_y  = (double) e_y/(double) n_y;
			// if (e_y == 0 || e_y == n_y)   { L_y = 0.0; }
			// else						{ L_y = kl_y/(double) n_y; }
			L_y = kl_y/(double) n_y;
			...
			n_x  = (n_j+n_k)*n_i;
			e_x  = internal[x].e + internal[y].e - e_y;						// e_yj
			kl_x = computeKL(internal[y].L->index, internal[y].L->type, internal[x].R->index, internal[x].R->type) + computeKL(internal[y].L->index, internal[y].L->type, internal[y].R->index, internal[y].R->type);
			p_x  = (double) e_x / (double) n_x;
			// if (e_x == 0 || e_x == n_x)	{ L_x = 0.0; }
			// else						{ L_x = kl_x / (double) n_x; }
			L_x = kl_x / (double) n_x;
			...
			p = min(1.00, exp(L)/exp(L+dLogL));
			// cout << dLogL << endl;
			// if ((dLogL < 0.0) || (mtr.randExc() < exp(dLogL))) {  // make LEFT BETA move
			if ((dLogL < 0.0) || (mtr.randExc() < p)) {  // make RIGHT BETA move
				ftaken = true;
		...
		if (mtr.randExc() < 0.5) {					// alpha move: (i,(j,k)) -> ((i,k),j)
			...
			kl_y  = computeKL(internal[x].L->index, internal[x].L->type, internal[y].R->index, internal[y].R->type);   // e_ik
			p_y  = (double) e_y/(double) n_y;
			// if (e_y == 0 || e_y == n_y)   { L_y = 0.0; }
			// else						{ L_y = kl_y/(double) n_y; }
			L_y = kl_y/(double) n_y;
			...
			n_x  = (n_i+n_k)*n_j;
			e_x  = internal[x].e + internal[y].e - e_y;						// e_yj
			kl_x = computeKL(internal[x].L->index, internal[x].L->type, internal[y].L->index, internal[y].L->type) + computeKL(internal[y].R->index, internal[y].R->type, internal[y].L->index, internal[y].L->type);
			p_x  = (double) e_x / (double) n_x;
			// if (e_x == 0 || e_x == n_x)	{ L_x = 0.0; }
			// else						{ L_x = kl_x/(double) n_x; }
			L_x = kl_x/(double) n_x;
			...
			p = min(1.00, exp(L)/exp(L+dLogL));
			// cout << dLogL << endl;
			// if ((dLogL < 0.0) || (mtr.randExc() < exp(dLogL))) {  // make RIGHT ALPHA move
			if ((dLogL < 0.0) || (mtr.randExc() < p)) {  // make RIGHT BETA move
				ftaken = true;
				d->swapEdges(x, internal[x].L->index, LEFT, x, internal[x].R->index, RIGHT);
				temp			   = internal[x].L;			// - swap L and R of [x]
			...
			kl_y  = computeKL(internal[x].L->index, internal[x].L->type, internal[y].L->index, internal[y].L->type);
			p_y  = (double) e_y/(double) n_y;
			// if (e_y == 0 || e_y == n_y)   { L_y = 0.0; }
			// else						{ L_y = kl_y / (double) n_y; }
			L_y = kl_y / (double) n_y;
			...
			p_x  = (double) e_x / (double) n_x;
			kl_x = computeKL(internal[x].L->index, internal[x].L->type, internal[y].R->index, internal[y].R->type) + computeKL(internal[y].L->index, internal[y].L->type, internal[y].R->index, internal[y].R->type);
			// if (e_x == 0 || e_x == n_x)	{ L_x = 0.0; }
			// else						{ L_x = kl_x / (double) n_x; }
			L_x = kl_x / (double) n_x; 
			// cout << "4. " << L_x << " " << L_y << endl;
...
void dendro::refreshLikelihood() {
	...
	for (int i=0; i<(n-1); i++) {
		...
		ei    = internal[i].e;
		internal[i].p = (double) ei/(double) nL_nR;
		dL = computeKL(internal[i].L->index, internal[i].L->type, internal[i].R->index, internal[i].R->type)/(double) nL_nR;
		// if (ei == 0 || ei == nL_nR) { dL = 0.0; }
		// else                        { dL = ei * log(internal[i].p) + (nL_nR - ei) * log(1.0-internal[i].p); }
		internal[i].logL = dL;
		L += dL;
