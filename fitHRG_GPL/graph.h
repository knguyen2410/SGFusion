...
edge::edge()  { x = -1; weight = 0; next = NULL; }
edge::~edge() { }
#endif
...
bool graph::addLink(const int i, const int j, const double z) {
...
double    graph::getWeight(const int i, const int j)       { 
	edge* curr;
	if (i >= 0 && i < n && j >= 0 && j < n) {
		curr = nodeLink[i];
		while (curr != NULL) {
			if (curr->x == j) { return curr->weight; }
			curr = curr->next;
		}
	} 
	return 0;
...
int    graph::getDegree(const int i)       { if (i >= 0 && i < n) { return nodes[i].degree; } else { return -1;   } }
string graph::getName(const int i)         { if (i >= 0 && i < n) { return nodes[i].name;   } else { return "";   } }
// NOTE: Returns address; deallocation of returned object is dangerous
edge*  graph::getNeighborList(const int i) { if (i >= 0 && i < n) { return nodeLink[i];     } else { return NULL; } }
...
bool graph::setName(const int i, const string text) { if (i >= 0 && i < n) { nodes[i].name = text; return true; } else { return false; } }
