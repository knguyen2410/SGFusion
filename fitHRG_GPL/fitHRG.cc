...
int main(int argc, char * argv[]) {
	...
	if (parseCommandLine(argc, argv)) {
		...
		if (ioparm.flag_f) {
			...
			// while (true) {
			// 	if (!(markovChainMonteCarlo())) { return 0; }
			// 	if (t >= 4294967294 || t < 0) { t = 1; }	// rollover step count
...
bool markovChainMonteCarlo() {
	...
	for (unsigned int i=0; i<period; i++) {
		...
		// if ((ioparm.flag_compact && ((i+(t-period)) % (8192) == 1)) || !ioparm.flag_compact) {
		// 	cout << "[" << t << "]\t" << Likeli[i];		// 
		// 	cout << "   \t(" << bestL << ")\t";			// 
		// 	if (flag_taken) { cout << "*\t"; } else { cout << " \t"; }
		// 	cout << "\t" << dL << endl;			// 
	...
	for (unsigned int i=0; i<period; i++) {
		if (ioparm.flag_compact && ((i+(t-period)) % (2048) == 1)) {
...
bool MCMCEquilibrium_Find() {
	...
	while (!flag_eq) {
		...
		for (int i=0; i<65536; i++) {
			...
			if (t > 2147483640 || t < 0) { t = 1; } else { t++; }
		}
		...
		if (fabs(newMeanL - oldMeanL)/65536.0 < 1.0 && (t>10000*ioparm.n)) { flag_eq = true; }
	}
	return true;
}
...
bool parseCommandLine(int argc, char * argv[]) {
	...
	if (argc==1) {
		...
		while (argct < argc) {
			...
			} else if (temp == "-f" && !ioparm.flag_make) {
...
bool readInputFile() {
	...
	while (fscan1 >> s >> f >> t ) {					// read friendship pair (s,f)
		if (s != f) {
	...
	while (fin >> s >> f >> t) {
		...
		if (s != f) {
			...
			if (!(d->g->doesLinkExist(a,b))) { if (!(d->g->addLink(a,b, t))) { cout << "Error: (" << s << " " << f << ")" << endl; } else if (d->g->getName(a) == "") { d->g->setName(a, num2str(s)); } }
			// if (!(d->g->doesLinkExist(b,a))) { if (!(d->g->addLink(b,a, t))) { cout << "Error: (" << s << " " << f << ")" << endl; } else if (d->g->getName(b) == "") { d->g->setName(b, num2str(f)); } }
		}		
		if (t2-t1>ioparm.timer) {				// check timer; if necessarsy, display
			cout << "   edges: ["<<m<<"]"<<endl;
			t1 = t2; ioparm.flag_timer = true;		// 
