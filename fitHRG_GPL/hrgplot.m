...
try
	[I,L,Ls,R,Rs,p,e,n,value] = textread(f1,'[ %u ] L= %u%s R= %u%s p= %n e=%u n=%u value =%f');
catch
...
if verbose
	for i=1:length(I)
		fprintf('[ %i ] L= %i %s R= %i %s p= %9.7f e= %i n= %i value= %f\n', ... 
			I(i),L(i),Ls{i},R(i),Rs{i},p(i),e(i),n(i),value(i));
	end;
end;
