...
class FedAvgZoneCross(Strategy):
    ...
    def configure_fit(
        ...
        if self.on_fit_config_fn is not None:
            ...
            self.config = self.on_fit_config_fn(rnd)
            self.config["country"] = self.country
            self.config["zid"] = self.zid
            self.config["lr"] = self.lr
            self.config["ustep"] = self.ustep
            self.config["beta"] = self.beta
            self.config["zid_cross"] = self.zid_cross
